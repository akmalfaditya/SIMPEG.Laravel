...
@section('content')
        ...
        <div class="p-5 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center gap-3">
            <div class="flex items-center gap-2 flex-wrap">
                <a href="{{ route('satyalencana.index', request()->only('search')) }}"
                    class="px-3 py-1.5 text-xs rounded-lg {{ !$selectedMilestone ? 'bg-blue-600 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200' }}">Semua</a>
                <a href="{{ route('satyalencana.index', array_merge(['milestone' => 10], request()->only('search'))) }}"
                    class="px-3 py-1.5 text-xs rounded-lg {{ $selectedMilestone == 10 ? 'bg-blue-600 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200' }}">10
                    Tahun</a>
                <a href="{{ route('satyalencana.index', array_merge(['milestone' => 20], request()->only('search'))) }}"
                    class="px-3 py-1.5 text-xs rounded-lg {{ $selectedMilestone == 20 ? 'bg-blue-600 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200' }}">20
                    Tahun</a>
                <a href="{{ route('satyalencana.index', array_merge(['milestone' => 30], request()->only('search'))) }}"
                    class="px-3 py-1.5 text-xs rounded-lg {{ $selectedMilestone == 30 ? 'bg-blue-600 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200' }}">30
                    Tahun</a>
            </div>
    ...
    <div id="award-modal" class="fixed inset-0 z-50 hidden">
        ...
        <div
            class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white rounded-2xl shadow-xl p-6 w-full max-w-md">
