...
<body class="h-full bg-slate-50 font-[Inter]">
    <div class="flex h-full">
        ...
        {{-- Sidebar --}}
        <aside id="sidebar" class="w-64 bg-gradient-to-b from-slate-900 to-slate-800 text-white flex flex-col shadow-2xl fixed h-full z-30 transition-all duration-300" style="transform: translateX(-100%);">
            ...
            <nav class="flex-1 overflow-y-auto py-4 px-3 space-y-1">
                ...
                <a href="{{ route('activity-log.index') }}" class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm transition-all duration-200 {{ request()->routeIs('activity-log.*') ? 'bg-blue-600/30 text-blue-300 font-medium' : 'text-slate-300 hover:bg-slate-700/50 hover:text-white' }}">
            ...
            <div class="p-4 border-t border-slate-700/60">
                <a href="{{ route('profile.show') }}" class="flex items-center gap-3 mb-3 px-1 py-1 rounded-lg hover:bg-slate-700/50 transition-all">
        ...
        <main class="flex-1 md:ml-64 overflow-y-auto">
            ...
            <div class="p-4 md:p-6">
                {{-- Flash Messages --}}
                @if(session('success'))
                <div class="mb-4 p-4 bg-emerald-50 border border-emerald-200 text-emerald-700 rounded-xl text-sm flex items-center gap-2 flash-toast" role="alert">
                ...
                @endif
                @if(session('error'))
                <div class="mb-4 p-4 bg-red-50 border border-red-200 text-red-700 rounded-xl text-sm flash-toast" role="alert">{{ session('error') }}</div>
                @endif
                @if($errors->any())
    ...
    <div id="delete-modal" class="fixed inset-0 z-50 hidden">
        ...
        <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white rounded-2xl shadow-xl p-6 w-full max-w-sm">
            ...
            <p class="text-sm text-slate-600 mb-5" id="delete-modal-message">Apakah Anda yakin ingin menghapus data ini? Tindakan ini tidak dapat dibatalkan.</p>
            <div class="flex gap-3 justify-end">
                <button onclick="closeDeleteModal()" class="px-4 py-2 text-sm text-slate-600 hover:text-slate-800 rounded-lg border border-slate-300 hover:bg-slate-50 transition-all">Batal</button>
                <form id="delete-modal-form" method="POST">
                    @csrf @method('DELETE')
                    <button type="submit" class="px-4 py-2 text-sm text-white bg-red-600 hover:bg-red-700 rounded-lg transition-all">Hapus</button>
                </form>
            </div>
        </div>
    </div>
