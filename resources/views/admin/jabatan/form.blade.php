    ...
    <form method="POST" action="{{ $jabatan ? route('admin.jabatan.update', $jabatan) : route('admin.jabatan.store') }}">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div class="md:col-span-2">
                <label class="block text-sm font-medium text-slate-700 mb-1">Nama Jabatan <span class="text-rose-500">*</span></label>
            ...
            {{-- Jenis Jabatan --}}
            <div>
                <label class="block text-sm font-medium text-slate-700 mb-1">Jenis Jabatan <span class="text-rose-500">*</span></label>
                <select name="jenis_jabatan" required class="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-800/30 focus:border-blue-800">
                ...
                @error('jenis_jabatan') <p class="text-xs text-rose-600 mt-1">{{ $message }}</p> @enderror
            </div>

            {{-- Rumpun --}}
            <div>
                <label class="block text-sm font-medium text-slate-700 mb-1">Rumpun <span class="text-rose-500">*</span></label>
                <select name="rumpun_jabatan_id" required class="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-800/30 focus:border-blue-800">
                ...
                @error('rumpun_jabatan_id') <p class="text-xs text-rose-600 mt-1">{{ $message }}</p> @enderror
            </div>

            {{-- BUP --}}
            <div>
                <label class="block text-sm font-medium text-slate-700 mb-1">BUP (Batas Usia Pensiun) <span class="text-rose-500">*</span></label>
                ...
                @error('bup') <p class="text-xs text-rose-600 mt-1">{{ $message }}</p> @enderror
            </div>

            {{-- Eselon Level --}}
            <div>
                <label class="block text-sm font-medium text-slate-700 mb-1">Level Eselon</label>
                <select name="eselon_level" class="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-800/30 focus:border-blue-800">
                ...
                @error('eselon_level') <p class="text-xs text-rose-600 mt-1">{{ $message }}</p> @enderror
            </div>
            ...
            <div>
                ...
                <input type="number" name="kelas_jabatan" value="{{ old('kelas_jabatan', $jabatan?->kelas_jabatan ?? 1) }}" min="1" max="17"
                    class="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-800/30 focus:border-blue-800">
                @error('kelas_jabatan') <p class="text-xs text-rose-600 mt-1">{{ $message }}</p> @enderror
            </div>
        </div>
        ...
        <div class="flex gap-3 mt-6 pt-4 border-t border-slate-200">
            ...
            <x-button variant="secondary" size="lg" href="{{ route('admin.jabatan.index') }}">Batal</x-button>
        </div>
    </form>
</x-card>
@endsection
