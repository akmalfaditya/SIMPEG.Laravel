...
<div class="bg-white rounded-xl border border-slate-200 shadow-sm">
    <div class="p-5 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center gap-3">
        <div class="flex items-center gap-2 flex-wrap flex-1">
            {{-- Filter Rumpun --}}
            <a href="{{ route('admin.jabatan.index', array_filter(['search' => $filterSearch, 'status' => $filterStatus])) }}" class="px-3 py-1.5 text-xs rounded-lg {{ $filterRumpun === null ? 'bg-blue-800 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200' }}">Semua</a>
            @foreach($rumpunList as $r)
                <a href="{{ route('admin.jabatan.index', array_filter(['rumpun' => $r->id, 'search' => $filterSearch, 'status' => $filterStatus])) }}" class="px-3 py-1.5 text-xs rounded-lg {{ (int) $filterRumpun === $r->id ? 'bg-blue-800 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200' }}">{{ $r->nama }}</a>
            @endforeach
            ...
            <a href="{{ route('admin.jabatan.index', array_filter(['rumpun' => $filterRumpun, 'search' => $filterSearch, 'status' => 'inactive'])) }}" class="px-3 py-1.5 text-xs rounded-lg {{ $filterStatus === 'inactive' ? 'bg-rose-600 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200' }}">Nonaktif</a>
        </div>
        <div class="flex items-center gap-2">
            <form method="GET" action="{{ route('admin.jabatan.index') }}" class="flex items-center gap-2">
                ...
                <input type="text" name="search" value="{{ $filterSearch }}" placeholder="Cari nama jabatan..." class="px-3 py-1.5 text-xs border border-slate-200 rounded-lg focus:ring-2 focus:ring-blue-800 focus:border-blue-800 w-48">
                <button type="submit" class="px-3 py-1.5 text-xs bg-slate-100 text-slate-600 rounded-lg hover:bg-slate-200">Cari</button>
            </form>
            <a href="{{ route('admin.jabatan.create') }}" class="px-3 py-1.5 bg-blue-800 hover:bg-blue-900 text-white text-xs font-medium rounded-lg transition-all flex items-center gap-1.5">
            ...
            <tbody class="divide-y divide-slate-100">
                ...
                <tr class="hover:bg-slate-50 transition-colors {{ !$j->is_active ? 'opacity-50' : '' }}">
                    ...
                    <td class="px-4 py-2.5 text-center">
                        ...
                        @else
                            <span class="px-2 py-0.5 text-xs rounded-full font-medium bg-rose-100 text-rose-700">Nonaktif</span>
                        @endif
                    </td>
                    <td class="px-4 py-2.5 text-center">
