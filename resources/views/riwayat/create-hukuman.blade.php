...
<div class="max-w-2xl bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
    <form method="POST" action="{{ route('riwayat.hukuman.store') }}" enctype="multipart/form-data">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div><label class="block text-sm font-medium text-slate-700 mb-1">Jenis Sanksi *</label><select name="jenis_sanksi" required class="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm">@foreach($sanksiOptions as $s)<option value="{{ $s->value }}" {{ old('jenis_sanksi') == $s->value ? 'selected' : '' }}>{{ $s->label() }}</option>@endforeach</select></div>
