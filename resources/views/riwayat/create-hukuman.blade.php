    ...
    <div class="max-w-2xl bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
        <form method="POST" action="{{ route('riwayat.hukuman.store') }}" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div><label class="block text-sm font-medium text-slate-700 mb-1">Tingkat Hukuman *</label><select
                        ...
                        @foreach ($tingkatOptions as $t)
                            <option value="{{ $t->value }}" {{ old('tingkat_hukuman') == $t->value ? 'selected' : '' }}>
                                {{ $t->label() }}</option>
                        @endforeach
                    </select>
                </div>
                <div><label class="block text-sm font-medium text-slate-700 mb-1">Jenis Sanksi *</label><select
                        ...
                        @foreach ($sanksiOptions as $s)
                            <option value="{{ $s->value }}" {{ old('jenis_sanksi') == $s->value ? 'selected' : '' }}>
                                {{ $s->label() }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="md:col-span-2"><label class="block text-sm font-medium text-slate-700 mb-1">Deskripsi</label>
                    <textarea name="deskripsi" rows="2" class="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm">{{ old('deskripsi') }}</textarea>
                </div>
                ...
                <div id="demotionPangkatSection"
                    ...
                    <select name="demotion_golongan_id" class="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm searchable-select">
                        ...
                        @foreach ($golonganOptions as $g)
                            <option value="{{ $g->id }}"
                                {{ old('demotion_golongan_id') == $g->id ? 'selected' : '' }}>{{ $g->label }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div id="demotionJabatanSection"
                    ...
                    <select name="demotion_jabatan_id" class="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm searchable-select">
                        ...
                        @foreach ($jabatanOptions as $j)
                            <option value="{{ $j->id }}"
                                {{ old('demotion_jabatan_id') == $j->id ? 'selected' : '' }}>{{ $j->nama_jabatan }}
                            </option>
                        @endforeach
                    </select>
                </div>

                <div><label class="block text-sm font-medium text-slate-700 mb-1">Upload SK Hukdis (PDF, maks
            ...
            <div class="flex gap-3 mt-6 pt-4 border-t border-slate-200">
                <button type="submit"
                    class="px-6 py-2.5 bg-blue-600 text-white text-sm font-medium rounded-xl">Simpan</button>
