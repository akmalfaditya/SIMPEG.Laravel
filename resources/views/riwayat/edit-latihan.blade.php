    ...
    <x-card class="max-w-2xl">
        <form method="POST" action="{{ route('riwayat.latihan.update', $riwayat) }}" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div><label class="block text-sm font-medium text-slate-700 mb-1">Nama Latihan *</label><input type="text"
                        name="nama_latihan" value="{{ $riwayat->nama_latihan }}" required
                ...
                <div><label class="block text-sm font-medium text-slate-700 mb-1">Tahun *</label><input type="number"
                        name="tahun_pelaksanaan" value="{{ $riwayat->tahun_pelaksanaan }}" required
                        class="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm"></div>
                <div><label class="block text-sm font-medium text-slate-700 mb-1">Jumlah Jam *</label><input type="number"
                        name="jumlah_jam" value="{{ $riwayat->jumlah_jam }}" required
                        class="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm"></div>
                <div><label class="block text-sm font-medium text-slate-700 mb-1">Penyelenggara</label><input type="text"
                        name="penyelenggara" value="{{ $riwayat->penyelenggara }}"
                        class="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm"></div>
                <div><label class="block text-sm font-medium text-slate-700 mb-1">Tempat</label><input type="text"
                        name="tempat_pelaksanaan" value="{{ $riwayat->tempat_pelaksanaan }}"
                        class="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm"></div>
                <div><label class="block text-sm font-medium text-slate-700 mb-1">No Sertifikat</label><input type="text"
                        name="no_sertifikat" value="{{ $riwayat->no_sertifikat }}"
                        class="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm"></div>
                <div><label class="block text-sm font-medium text-slate-700 mb-1">Upload Sertifikat (PDF, maks
                    ...
                    @if ($riwayat->file_pdf_path)
                        <p class="text-xs text-green-600 mt-1">File sudah ada. Upload baru akan mengganti file lama. <a
                                href="{{ route('dokumen.download', ['type' => 'latihan', 'id' => $riwayat->id]) }}"
