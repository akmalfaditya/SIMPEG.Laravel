    ...
    <div class="max-w-2xl bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
        <form method="POST" action="{{ route('riwayat.kgb.update', $riwayat) }}" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div><label class="block text-sm font-medium text-slate-700 mb-1">TMT KGB *<x-tooltip text="Terhitung Mulai Tanggal kenaikan gaji berkala" /></label><input type="date"
                        name="tmt_kgb" value="{{ $riwayat->tmt_kgb->format('Y-m-d') }}" required
                        class="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm"></div>
                <div><label class="block text-sm font-medium text-slate-700 mb-1">Gaji Lama *</label><input type="number"
                        name="gaji_lama" value="{{ $riwayat->gaji_lama }}" required
                        class="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm"></div>
                <div><label class="block text-sm font-medium text-slate-700 mb-1">Gaji Baru *</label><input type="number"
                        name="gaji_baru" value="{{ $riwayat->gaji_baru }}" required
                        class="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm"></div>
                <div><label class="block text-sm font-medium text-slate-700 mb-1">Masa Kerja Gol. (Tahun)</label><input
                        type="number" name="masa_kerja_golongan_tahun" value="{{ $riwayat->masa_kerja_golongan_tahun }}"
                        required class="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm"></div>
                <div><label class="block text-sm font-medium text-slate-700 mb-1">Masa Kerja Gol. (Bulan)</label><input
                        type="number" name="masa_kerja_golongan_bulan" value="{{ $riwayat->masa_kerja_golongan_bulan }}"
                        required class="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm"></div>
                <div><label class="block text-sm font-medium text-slate-700 mb-1">Upload SK (PDF, maks 5MB)</label><input
                    ...
                    @if ($riwayat->file_pdf_path)
                        <p class="text-xs text-green-600 mt-1">File SK sudah ada. Upload baru akan mengganti file lama. <a
                                href="{{ route('dokumen.download', ['type' => 'kgb', 'id' => $riwayat->id]) }}"
