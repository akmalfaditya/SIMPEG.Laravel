...
<div class="max-w-2xl bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
    <form method="POST" action="{{ route('riwayat.pendidikan.store') }}" enctype="multipart/form-data">
        @csrf
        <input type="hidden" name="pegawai_id" value="{{ $pegawaiId }}">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div><label class="block text-sm font-medium text-slate-700 mb-1">Tingkat Pendidikan *</label><select name="pendidikan_id" required class="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm">
                @foreach($pendidikanList as $p)
                    <option value="{{ $p?->id }}" {{ $p?->nama === 'S1' ? 'selected' : '' }}>{{ $p?->nama }}</option>
                @endforeach
            </select></div>
