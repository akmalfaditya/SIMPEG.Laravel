        ...
        <div class="p-5 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center gap-3">
            <div class="flex items-center gap-2 flex-wrap">
                <a href="{{ route('pensiun.index', request()->only('search')) }}"
                    class="px-3 py-1.5 text-xs rounded-lg {{ !($filterLevel ?? null) ? 'bg-blue-600 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200' }}">Semua</a>
                <a href="{{ route('pensiun.index', array_merge(['level' => 'Hitam'], request()->only('search'))) }}"
                    class="px-3 py-1.5 text-xs rounded-lg {{ ($filterLevel ?? null) === 'Hitam' ? 'bg-blue-600 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200' }}">Hitam
                    (Lewat)</a>
                <a href="{{ route('pensiun.index', array_merge(['level' => 'Merah'], request()->only('search'))) }}"
                    class="px-3 py-1.5 text-xs rounded-lg {{ ($filterLevel ?? null) === 'Merah' ? 'bg-blue-600 text-white' : 'bg-red-50 text-red-600 hover:bg-red-100' }}">Merah
                    (≤6 bln)</a>
                <a href="{{ route('pensiun.index', array_merge(['level' => 'Kuning'], request()->only('search'))) }}"
                    class="px-3 py-1.5 text-xs rounded-lg {{ ($filterLevel ?? null) === 'Kuning' ? 'bg-blue-600 text-white' : 'bg-yellow-50 text-yellow-700 hover:bg-yellow-100' }}">Kuning
                    (≤12 bln)</a>
                <a href="{{ route('pensiun.index', array_merge(['level' => 'Hijau'], request()->only('search'))) }}"
                    class="px-3 py-1.5 text-xs rounded-lg {{ ($filterLevel ?? null) === 'Hijau' ? 'bg-blue-600 text-white' : 'bg-green-50 text-green-700 hover:bg-green-100' }}">Hijau
                    (≤24 bln)</a>
            </div>
