    ...
    <div class="overflow-x-auto">
        <table class="w-full text-sm" id="data-table">
            ...
            <tbody class="divide-y divide-slate-100" id="table-body">
                ...
                <td class="px-4 py-2.5"><span class="px-2 py-1 text-xs rounded-full font-medium {{ $colors[$a['alert_level']] ?? '' }}">{{ $a['alert_level'] }}</span></td>
                <td class="px-4 py-2.5 text-center">
                    @if(in_array($a['alert_level'], ['Hitam', 'Merah']))
                    <a href="{{ route('pensiun.process.form', $a['pegawai_id']) }}" class="inline-flex items-center px-3 py-1.5 text-xs font-medium bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 transition-colors">
