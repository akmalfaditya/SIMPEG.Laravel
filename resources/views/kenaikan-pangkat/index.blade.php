...
<div class="bg-white rounded-2xl border border-slate-200 shadow-sm">
    <div class="p-5 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center gap-3">
        <div class="flex items-center gap-2 flex-wrap">
            <a href="{{ route('kenaikan-pangkat.index') }}" class="px-3 py-1.5 text-xs rounded-lg {{ $activeFilter === 'semua' ? 'bg-blue-600 text-white' : 'bg-slate-100 text-slate-600' }}">Semua</a>
            <a href="{{ route('kenaikan-pangkat.eligible') }}" class="px-3 py-1.5 text-xs rounded-lg {{ $activeFilter === 'eligible' ? 'bg-blue-600 text-white' : 'bg-slate-100 text-slate-600' }}">Eligible</a>
            <a href="{{ route('kenaikan-pangkat.ditunda') }}" class="px-3 py-1.5 text-xs rounded-lg {{ $activeFilter === 'ditunda' ? 'bg-red-600 text-white' : 'bg-red-50 text-red-600' }}">Ditunda (Hukdis)</a>
        </div>
    ...
    <div class="overflow-x-auto">
        <table class="w-full text-sm" id="data-table">
            ...
            <tbody class="divide-y divide-slate-100" id="table-body">
                ...
                <td class="px-4 py-2.5"><span class="px-2 py-1 text-xs rounded-full font-medium {{ $c['is_eligible'] ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-700' }}">{{ $c['is_eligible'] ? 'Eligible' : 'Belum' }}</span>@if($c['hukdis_pangkat_flag'])<div class="mt-1"><span class="px-2 py-0.5 text-[10px] rounded-full font-bold bg-red-600 text-white">{{ $c['hukdis_pangkat_note'] }}</span></div>@endif</td><td class="px-4 py-2.5">@if($c['is_eligible'] && !$c['hukdis_pangkat_flag'])<a href="{{ route('kenaikan-pangkat.process.form', $c['pegawai_id']) }}" class="px-3 py-1.5 text-xs bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors">Proses</a>@endif</td></tr>
                @empty <tr class="empty-row"><td colspan="11" class="px-4 py-8 text-center text-slate-400">Tidak ada data.</td></tr> @endforelse
            </tbody>
        </table>
    </div>
