    ...
    <div class="overflow-x-auto">
        <table class="w-full text-sm" id="data-table">
            ...
            <tbody class="divide-y divide-slate-100" id="table-body">
                ...
                <td class="px-4 py-2.5"><span class="px-2 py-1 text-xs rounded-full font-medium {{ $c['is_eligible'] ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-700' }}">{{ $c['is_eligible'] ? 'Eligible' : 'Belum' }}</span>@if($c['hukdis_pangkat_flag'])<div class="mt-1"><span class="px-2 py-0.5 text-[10px] rounded-full font-bold bg-red-600 text-white">{{ $c['hukdis_pangkat_note'] }}</span></div>@endif</td></tr>
                @empty <tr class="empty-row"><td colspan="10" class="px-4 py-8 text-center text-slate-400">Tidak ada data.</td></tr> @endforelse
            </tbody>
        </table>
    </div>
